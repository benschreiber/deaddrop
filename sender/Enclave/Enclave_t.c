...
typedef struct ms_encrypt_msg_t {
	uint8_t* ms_buf;
	size_t ms_len;
} ms_encrypt_msg_t;
...
static sgx_status_t SGX_CDECL sgx_encrypt_msg(void* pms)
{
	ms_encrypt_msg_t* ms = SGX_CAST(ms_encrypt_msg_t*, pms);
	...
	CHECK_REF_POINTER(pms, sizeof(ms_encrypt_msg_t));
...
SGX_EXTERNC const struct {
	...
	3,
	{
		{(void*)(uintptr_t)sgx_encrypt_msg, 0},
...
sgx_status_t SGX_CDECL ocall_result(uint8_t* cipher)
{
	sgx_status_t status = SGX_SUCCESS;
	size_t _len_cipher = cipher ? strlen(cipher) + 1 : 0;

	ms_ocall_result_t* ms = NULL;
	size_t ocalloc_size = sizeof(ms_ocall_result_t);
	void *__tmp = NULL;

	ocalloc_size += (cipher != NULL && sgx_is_within_enclave(cipher, _len_cipher)) ? _len_cipher : 0;
	...
	ms = (ms_ocall_result_t*)__tmp;
	__tmp = (void *)((size_t)__tmp + sizeof(ms_ocall_result_t));

	if (cipher != NULL && sgx_is_within_enclave(cipher, _len_cipher)) {
		ms->ms_cipher = (uint8_t*)__tmp;
		__tmp = (void *)((size_t)__tmp + _len_cipher);
		memcpy(ms->ms_cipher, cipher, _len_cipher);
	} else if (cipher == NULL) {
		ms->ms_cipher = NULL;
	} else {
		sgx_ocfree();
